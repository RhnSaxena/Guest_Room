...
if(isset($_POST["checkin"]) && !empty($_POST["checkin"]) && isset($_POST["checkout"]) && !empty($_POST["checkout"])){
	...
	if(strtotime($_SESSION['checkin_date']) > strtotime($_SESSION['checkout_date'])){
	
	
	echo ("<script LANGUAGE='JavaScript'>
    window.alert('Please Enter Correct Date');
    window.location.href='index.php';
    </script>");

	}
	$now=Date("Y-m-d");
	if(strtotime($now) > strtotime($_SESSION['checkout_date'])){
	...
	if(strtotime($now) > strtotime($_SESSION['checkin_date'])){
		...
		<p><b>Your Reservation</b></p><hr class="line">
				...
				<div class="row">
					<div class="large-12 columns">
						...
						<div class="row">
							<div class="large-6 columns" style="max-width:100%;">
								<span class="fontgrey">Total Persons
								</span>
							</div>
