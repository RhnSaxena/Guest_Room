...
include './auth.php';
	...
	if(!empty($_POST["checkin"]) && !empty($_POST["checkout"])){
					$_SESSION['result'] = mysqli_query($connection,"SELECT * from booking where
						((booking.checkin_date between '".$datestart."' AND '".$dateend."') OR (booking.checkout_date between '".$dateend."' AND '".$datestart."'))
						");
	}
	else {
			$_SESSION['result'] =  mysqli_query($connection,"select booking.*  from booking;");	echo mysqli_error($connection);			
	}	
